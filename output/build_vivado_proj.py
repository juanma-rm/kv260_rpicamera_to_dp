...
class VivadoBuilder:
    ...
    def generate_dtbo(self) -> bool:
        ...
        try:
            ...
            # 2. Locate the generated DTBO
            platform_dir = self.dtbo_path / "vitis_ws" / "p"
            dtbo_files = list(platform_dir.glob("**/pl.dtbo")) + list(platform_dir.glob("**/overlay.dtbo"))
            
            if not dtbo_files:
                # If .dtbo not found, look for .dtsi files generated by SDTgen
                # In 2025.2 these are often in <platform>/export/p/sw/p/ovl/device-tree/
                dtsi_files = list(platform_dir.glob("**/pl.dtsi")) + list(platform_dir.glob("**/system-top.dtsi"))
                if not dtsi_files:
                    print("Error: Could not find generated DTBO or DTSI in platform component.")
                    return False
                
                print(f"Found DTSI at: {dtsi_files[0]}. Compiling manually...")
                dtc_exe = self.get_tool_path("dtc")
                dtbo_out = self.artifacts_path / f"{self.PROJECT_NAME}.dtbo"
                cmd = [dtc_exe, "-@", "-O", "dtb", "-o", str(dtbo_out), str(dtsi_files[0])]
                subprocess.run(cmd, check=True)
            else:
                print(f"Found generated DTBO at: {dtbo_files[0]}")
                shutil.copy2(dtbo_files[0], self.artifacts_path / f"{self.PROJECT_NAME}.dtbo")
